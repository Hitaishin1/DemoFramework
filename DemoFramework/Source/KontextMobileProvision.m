...
@implementation KontextMobileProvision
...
+ (UIApplicationReleaseMode) releaseMode {
    ...
    if (mobileProvision) {
        [Kontext kontext_Log:KON_TEXT_LL_DEBUG message:[NSString stringWithFormat:@"mobileProvision: %@", mobileProvision]];
        entitlements = [mobileProvision objectForKey:@"Entitlements"];
    }
    else
        [Kontext kontext_Log:KON_TEXT_LL_DEBUG message:@"mobileProvision not found"];
